...
def run_legacy_confusion_matrix_cleanup() -> int:
    """Remove legacy generic confusion matrix artifacts without touching run-scoped files."""
    du.print_section("Cleanup Legacy Confusion Matrices")
    conf_dir = Path(app_config.DEFAULT_OUTPUT_DIR) / "conf_matrices"
    if not conf_dir.exists():
        du.print_info(f"[MENU] No confusion matrix directory found: {conf_dir}")
        return 0

    legacy_names = {
        "confusion_matrix_random_forest.png",
        "confusion_matrix_balanced_random_forest.png",
        "confusion_matrix_svm.png",
        "confusion_matrix_xgboost.png",
        "confusion_matrix_logistic_regression.png",
    }
    targets = [path for path in conf_dir.glob("confusion_matrix_*.png") if path.name in legacy_names]
    if not targets:
        du.print_info("[MENU] No legacy generic confusion matrix files detected.")
        return 0

    du.print_info(f"[MENU] Found {len(targets)} legacy file(s):")
    du.print_table(
        [{"filename": p.name, "path": str(p)} for p in sorted(targets, key=lambda x: x.name)],
        title="Legacy confusion matrix candidates",
        show_index=False,
        max_rows=25,
    )
    if not mu.confirm_prompt("Delete these legacy confusion matrix files now?"):
        du.print_info("[MENU] No files deleted.")
        return 0
    ...
    for path in targets:
        try:
            ...
            du.print_warning(f"[MENU] Failed to delete {path.name}: {exc}")
    du.print_success(f"[MENU] Removed {removed}/{len(targets)} legacy confusion matrix file(s).")
    return 0


def run_legacy_model_export_cleanup() -> int:
    """Remove legacy top-level model exports under output/models/*."""
    du.print_section("Cleanup Legacy Model Exports")
    output_root = Path(app_config.DEFAULT_OUTPUT_DIR)
    legacy_models_dir = output_root / "models"
    if not legacy_models_dir.exists():
        du.print_info(f"[MENU] No legacy models directory found: {legacy_models_dir}")
        return 0

    targets: list[Path] = []
    for path in legacy_models_dir.rglob("*"):
        if path.is_file():
            targets.append(path)

    if not targets:
        du.print_info("[MENU] No legacy model export files detected.")
        return 0

    du.print_info(f"[MENU] Found {len(targets)} legacy model file(s).")
    du.print_table(
        [{"path": str(p)} for p in sorted(targets, key=lambda x: str(x))],
        title="Legacy model export candidates",
        show_index=False,
        max_rows=20,
    )
    if not mu.confirm_prompt("Delete these legacy model export files now?"):
        du.print_info("[MENU] No files deleted.")
        return 0
    ...
    for path in targets:
        try:
            ...
            du.print_warning(f"[MENU] Failed to delete {path}: {exc}")

    for directory in sorted(legacy_models_dir.rglob("*"), key=lambda x: len(str(x)), reverse=True):
        if directory.is_dir():
            try:
                directory.rmdir()
            except Exception:
    ...
    du.print_success(f"[MENU] Removed {removed}/{len(targets)} legacy model export file(s).")
    return 0
