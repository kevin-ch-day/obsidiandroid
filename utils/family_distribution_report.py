# Filename: family_distribution_report.py
# Purpose : Standalone utility to print and export malware family distribution stats before training.
...
# === Public Entry Point ===
def print_family_distribution_stats(samples_df: pd.DataFrame):
    """
    Main controller for analyzing and exporting family distribution.

    Args:
        samples_df (pd.DataFrame): DataFrame that includes a 'family_name' column.
    """
...
def _display_family_distribution_console(fam_counts: Counter):
    ...
    if low_support:
        du.print_warning(f"{len(low_support)} families have <=3 samples. These may affect classifier reliability.")
...
def _display_family_group(group: dict, highlight: bool = False):
...
# === File Export Logic ===
def _export_family_distribution_report(fam_counts: Counter):
    try:
...
def _write_report_to_disk(path: Path, content: str):
