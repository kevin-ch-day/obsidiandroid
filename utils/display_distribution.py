...
def _print_distribution_header(title: str):
    print(f"\n{'=' * 90}")
...
def _print_distribution_row(category: str, count: int, percent: float, bar: str):
    print(f"{category:<30} {count:>8}  {percent:6.2f}%  {bar}")

def _print_distribution_footer(total: int):
    print(f"{'-' * 90}")
    print(f"{'Total':<30} {total:>8}  {'100.00%':>6}")
    print("→ Insight: Use for class balance, anomaly prevalence, or segmentation strategies.\n")

def print_distribution(series: pd.Series, label: str = "Category Distribution", scale: int = 50, bar_char: str = "█"):
    if series.empty:
