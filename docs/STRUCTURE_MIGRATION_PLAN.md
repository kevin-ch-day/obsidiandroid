...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Through **Pass 93**, the **training execution stack** (**`train_model_executor`**, **`model_training`**, **`training_helpers`**, **`prediction_builder`**, **`model_evaluation`**, **`pipeline_result_promoter`**, **`ml_trainers/*`**) is physically under **`obsidiandroid.modeling`** with thin **`ml_classification.training.*`** shims; **`ml_classification.common`**, deeper **`ml_classification.labeling`**, and **`ml_classification.ml_utils`** remnants still separate from this slice |
