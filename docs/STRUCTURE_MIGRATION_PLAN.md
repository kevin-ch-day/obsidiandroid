...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Through **Pass 97**: **`obsidiandroid.inference`** (**consensus**, threat/type engines, signal health); builders/labeling/reporting import canonically; remaining **`engine_weights/`**, optional **`compile_classification_results`** physical move, **`ml_classification/reporting`** other leaves, etc. |
