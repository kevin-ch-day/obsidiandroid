...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Baseline façades/adoption **46–50A**, **58**, **77**; **Pass 82–83** features/vectorization; **Passes 85–86** labeling constants + resolver; **Passes 87–91** modeling/result helpers + **`feature_alignment_utils`**, **`feature_schema_audit`** (under **`obsidiandroid.features`**), **`data_alignment`**, **`ml_result_*`**, **`model_prediction`** physically canonical with legacy shims; large training orchestration (**`pipeline_core`**, **`model_trainer_factory`**, trainers, **`train_model_executor`**, …) still **`ml_classification.training`** |
