...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Through **Pass 98**: **`obsidiandroid.engine_weights`** (six modules); **`compile_classification_results`** under **`obsidiandroid.reporting`**; **`ml_classification.reporting`** now only **`ml_report_builder`** shim (**Pass 94** canonical **`obsidiandroid.evaluation`**). Remaining **`ml_classification`** mostly empty package shells + historical **`common`** shims. |
