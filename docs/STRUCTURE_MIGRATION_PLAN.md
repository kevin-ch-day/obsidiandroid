...
- **Retirement matrix:** the current status/blockers for the three remaining legacy compatibility trees (**`analysis`**, **`ml_classification`**, **`database`**) is tracked in **`scripts/dev/compatibility_retirement_manifest.py`** and printable via **`python scripts/dev/report_compatibility_retirement.py`**.
...
- **Legacy trees** at the repo root (**`analysis/`**, **`ml_classification/`**) stay primarily for compatibility shims and legacy import identity; repo-root **`database/`** holds **thin shims** (and the **`split_db_health`** `__main__` entry) for legacy **`database.*`** paths; DB implementations for those names live under **`src/obsidiandroid/database/`** behind the curated **`obsidiandroid.database`** façade. As of the latest repo audit there are still roughly **155** Python files across these legacy trees, so the remaining migration burden is mostly **retirement / deprecation planning**, not discovery of missing canonical homes.
...
| Item | Notes |
|------|-------|
| **`src/obsidiandroid/diagnostics/__init__.py`** | Historical early façade step. Current state is stronger: diagnostics implementation is canonical under **`src/obsidiandroid/diagnostics/`** and **`analysis.diagnostics.*`** is the compatibility shim surface. |
