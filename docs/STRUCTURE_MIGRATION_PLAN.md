...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Training **Pass 93**, eval **Pass 94**, labeling **Pass 95**, structured row builder package **Pass 96** (**`obsidiandroid.classification_builder`**); remaining **`inference/`**, **`engine_weights/`**, **`ml_classification/reporting/compile_classification_results`** (caller updated), deep **`inference`** inside **`label_field_normalizer`**, etc. |
