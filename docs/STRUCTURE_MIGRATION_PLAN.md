...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Passes **46–50A** facade/adoption; **Pass 58** adds **`obsidiandroid.labeling.taxonomy`** wrapper (not raw-alias) for family normalization; **Pass 77** migrates **`runner`**, diagnostics **`output_artifact_policy`** producer strings, and selected tests to **`obsidiandroid.modeling`** / **`features`** where Pass **47** aliases already match **`ml_classification`** modules |
...
| `obsidiandroid/features/` | **partial** | ML façade re-exports (Passes **46–48**); bulk of vectorization still **`ml_classification/`**. |
