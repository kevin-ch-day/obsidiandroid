...
- **Single source for retirement metadata:** canonical-relocation completion domains and retired compatibility paths live in **`scripts/dev/compatibility_retirement_manifest.py`** so guardrails and future retirement tooling do not drift.
- **Retirement matrix:** **`python scripts/dev/report_compatibility_retirement.py`** reports the active retirement state; it is intentionally empty once all repo-root Python compatibility trees are gone.
