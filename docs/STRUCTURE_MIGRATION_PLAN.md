...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Baseline façades/adoption **46–50A**, **58**, **77**; **Pass 82–83** features/vectorization; **Passes 85–86** labeling constants + resolver; **Passes 87–92** modeling/result/helpers + **`pipeline_core`** / **`model_trainer_factory`**, **`feature_alignment_utils`**, **`feature_schema_audit`** (under **`obsidiandroid.features`**), **`data_alignment`**, **`ml_result_*`**, **`model_prediction`** physically canonical with legacy shims; remaining training execution (**`train_model_executor`**, **`model_training`**, **`training_helpers`**, promotion/eval leaves, …) mostly **`ml_classification.training`** |
...
- **Pass 92 (training controller physical slice):** **`pipeline_core`** and **`model_trainer_factory`** live under **`obsidiandroid.modeling`**; canonical modules import remaining execution helpers from **`ml_classification.training`**; legacy **`ml_classification.training.pipeline_core`** / **`model_trainer_factory`** are thin identity shims.
...
| `obsidiandroid/labeling/` | **partial** | **`malware_family_constants`** + **`classification_label_resolver`** canonical (**Passes 85–86**); **`taxonomy`** wrapper (**Pass 58**); deeper **`ml_classification/labeling/`** helpers **move_later**. |
| `obsidiandroid/modeling/` | **partial** | **`model_exporter`** canonical; **`data_alignment`**, **`distribution_reporter`**, **`feature_label_alignment_helper`**, **`feature_alignment_utils`**, **`ml_result_analyzer`**, **`ml_result_validator`**, **`model_prediction`**, **`pipeline_core`**, **`model_trainer_factory`** canonical (**Passes 87–88, 90–92**); remainder **`ml_classification/training/`** (executor, training loop, helpers, promotion). |
...
| `ml_classification/training/` | `obsidiandroid.modeling` | **partial** — large leaves remain; controllers moved **Pass 92** |
