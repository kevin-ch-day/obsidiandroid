...
| P2 | Vendor / evaluation domain boundary (**`obsidiandroid.vendors`**, **`obsidiandroid.evaluation`**) | **Mostly done** — implementations are canonical (Passes **59**, **63**, **64**) and stable entrypoints exist (**`obsidiandroid.vendors.parse_generic_classification`**, **`obsidiandroid.evaluation.parse_vendor_classifications`**) with CI guardrails. Remaining work is *contract freezing* (typed results / schema) before widening public API further. |
...
| `obsidiandroid/database/` | **partial** (Pass 38+) | Curated façade re-exports match repo-root **`database.*`** shims (same **`ModuleType`**); implementations live under **`src/obsidiandroid/database/`**; ordered exports + CI parity: **`facade_manifest`**. |
