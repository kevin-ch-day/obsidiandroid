...
- **Legacy trees** at the repo root (**`analysis/`**, **`ml_classification/`**) stay primarily for compatibility shims and legacy import identity; repo-root **`database/`** remains an intentional implementation tree behind the curated **`obsidiandroid.database`** façade.
...
- **Quality gates:** **`make ci`** runs doc hygiene, **`scripts/dev/check_import_surface.py`** (imports, UTF-8 BOM scan, legacy leaf shim scan), fast pytest, and strict ML static scan.
- **Import-surface guard:** **`scripts/dev/check_import_surface.py`** rejects new imports from **`analysis`** or **`ml_classification`** inside canonical **`src/`** code, normal **`scripts/`**, and non-parity tests. Explicit shim/parity tests and the check script remain allowlisted because they verify legacy shim identity. The same guard rejects stale legacy **`# Filename:`** headers in canonical **`src/`** modules and enforces non-**`__init__`** legacy leaves under **`analysis/`** and **`ml_classification/`** as thin **`sys.modules`** identity shims.
