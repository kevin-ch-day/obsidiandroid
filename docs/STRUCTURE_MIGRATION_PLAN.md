...
| `src/obsidiandroid/pipeline/__init__.py` | **moved_now** | Canonical pipeline namespace and public import surface; legacy **`analysis.pipeline.*`** leaves are thin identity shims to the canonical modules. |
...
| `obsidiandroid/governance/` | **moved_now** (canonical) | Pipeline governance leaves (**exceptions**, **integrity**, **policy**, **readiness**), compliance, manifest, cohort, evidence mode, and artifacts live under **`src/`**. Remaining **`analysis.pipeline.governance.*`** paths are import-identity shims only. |
...
| `analysis/pipeline/governance/*` | `obsidiandroid.governance` | **moved_now** (**Pass 75**) — **`exceptions`**, **`integrity`**, **`policy`**, **`readiness`** under **`src/`**; **`analysis/pipeline/governance/`** leaf shims + lazy package **`__init__.py`**. |
...
| Gate | Requirement |
|------|----------------|
| **Import surface** | **`scripts/dev/check_import_surface.py`** asserts each façade export **`is`** the **`database.*`** counterpart; optional helpers use the same identity check via **`facade_manifest.LEGACY_SHIM_PAIRS`**. |
