...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Partial** — Training stack **Pass 93** (**`obsidiandroid.modeling`**) + **Pass 94** closures: classifier eval (**`ml_eval_engine`**, **`ml_comparator_summary`**, tiers + **`ml_report_builder`**) under **`obsidiandroid.evaluation`**, **`dataset_splitter`** under **`obsidiandroid.modeling`**; remaining subtrees **`ml_classification.builder`**, **`inference/`**, **`labeling/`** depth, **`engine_weights/`**, **`compile_classification_results`**, etc. |
