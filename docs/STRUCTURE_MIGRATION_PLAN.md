...
- **Canonical code** under **`src/obsidiandroid/`** includes **CLI**, **common** (hashing, paths, exports, hygiene, …), **governance** (compliance, manifest, cohort, artifacts, …), **reporting** (notably **`export_manager`** and related exporters), **observability** (**`logging/`** + **`pipeline_observability`**), **`pipeline`** façade: **`run_pipeline`**, every stage module **`runner`** imports directly (**Pass 70**), **`sample_preparation`**, **`sample_exports`**, **`main_facade`**, and policy leaves — each with legacy **`analysis.pipeline.*`** compatibility aliases where moved (**Passes 66–70**, later narrowed to the final protected shell); **`runner`** imports **`reset_runtime_training_caches`** from **`obsidiandroid.modeling.model_trainer_factory`** (**Pass 77**); vendor/feature-engineering helpers live under **`obsidiandroid.feature_engineering`** (**Pass 78**); pipeline orchestration surface (**permission features**, **`runtime_reporting`**, methodology exports, profile filters, metadata shim) lives under **`obsidiandroid.orchestration`**, and AV binary-matrix / enrichment under **`obsidiandroid.matrix`** (**Pass 80**). **diagnostics** under **`obsidiandroid.diagnostics`** (**`research_validity/`**, **`hostile_audit/`**, and leaf run diagnostics modules; Pass **65**), **database** façade → **`obsidiandroid.database`** re-exports curated modules (Pass 38+); implementations live under **`src/obsidiandroid/database/`**; repo-root **`database/<name>.py`** are thin identity shims (except **`split_db_health`**, which also wires **`python -m database.split_db_health`**). Façade order and legacy-shim parity tuples: **`obsidiandroid.database.facade_manifest`**; **modeling** (**`model_exporter`**, **`distribution_reporter`**, **`feature_label_alignment_helper`**), **evaluation** implementation under **`obsidiandroid.evaluation`** (Pass 63), **vendor parsing** under **`obsidiandroid.vendors.parsing`** (Pass 59), **vendor contracts** under **`obsidiandroid.vendors.contracts`** (including record diagnostics / metadata normalization helper closure in **Pass 84**), and **vendor execution** (parser runtime) under **`obsidiandroid.vendors.execution`** (Pass 64). **`obsidiandroid.risk_band`** owns risk-band assignment plus config (**Passes 81, 84**). **`obsidiandroid.labeling.malware_family_constants`** owns malware-family taxonomy constants/helpers, **`obsidiandroid.labeling.classification_label_resolver`** owns the stable resolver entrypoint, and **`obsidiandroid.labeling.taxonomy`** remains the function-level wrapper for public normalization use. **`obsidiandroid.features.vectorization`** holds the four vectorization modules physically (**Pass 83**); **`obsidiandroid.features`** façade re-exports them. The repo-root **`ml_classification/`** compatibility tree is retired. Remaining sizable moves under **`obsidiandroid.*`** are optional doc/caller cleanups.
- **Migration phase (2026-05):** The repository is now primarily in a **compatibility-retirement / supportability** phase rather than a large-scale **code relocation** phase. The highest remaining engineering work is: (a) reducing shim dependence in tests and legacy callers, (b) simplifying the CLI/control layer so operator UX does not mirror old subsystem boundaries, and (c) pruning legacy output compatibility such as **`paper2_pack`**, duplicated split-audit exports, and parallel **`paper_mode`** / **`evidence_mode`** language.
- **Canonical relocation complete for core domains:** The following domains should now be treated as physically relocated and canonical under **`src/obsidiandroid/`**: **pipeline**, **common**, **governance**, **observability**, **diagnostics**, **database**, **modeling**, **evaluation**, **reporting**, **vendors**, **features**, **labeling**, **classification_builder**, **inference**, **engine_weights**, **risk_band**, **matrix**, and **orchestration**. Remaining work in these areas is about **shim retirement**, **API boundary cleanup**, or **docs/tests**, not moving implementation files.
- **Single source for retirement metadata:** canonical-relocation completion domains, legacy compatibility roots, shim-tree roots, and parity-test allowlists now live in **`scripts/dev/compatibility_retirement_manifest.py`** so guardrails and future retirement tooling do not drift.
- **Retirement matrix:** the current status/blockers for the remaining legacy compatibility trees (**`analysis`** and **`database`**) is tracked in **`scripts/dev/compatibility_retirement_manifest.py`** and printable via **`python scripts/dev/report_compatibility_retirement.py`**.
- **Subtree retirement buckets:** the same manifest now distinguishes package-only shim namespaces, leaf identity shim batches, lazy compatibility facades, and entrypoint-compatibility shims. Each bucket records its **canonical target** under **`src/obsidiandroid/`** plus a validated Python file count so retirement work can be planned as explicit batches rather than by ad hoc grep.
- **Opt-in deprecation preview:** ready-now legacy shim batches now support **`OBSIDIANDROID_WARN_LEGACY_SHIMS=1`** so developers can surface import-time warnings while auditing or migrating callers without changing default runtime behavior.
- **Standardized legacy shim classes:** ordinary leaves under **`analysis/pipeline`** and repo-root **`database/*.py`** use the shared helpers in **`obsidiandroid.legacy_shim_lazy`** with static guardrails in **`scripts/dev/check_import_surface.py`**. Remaining relocation work in those trees is primarily deprecation planning, parity narrowing, or entrypoint preservation rather than shim-shape cleanup.
- **CLI/control layer:** The canonical CLI is under **`src/obsidiandroid/cli/`** and recent support passes have already split some high-churn surfaces into smaller modules (for example **`startup_menu_diagnostics`**, **`menu/operator_state`**, **`menu/vendor_parser_state`**, and **`profile_selection`**). The remaining support hotspot is not package placement but **responsibility overlap**: **`startup_menu.py`** is still large, and several intent areas still recompute mini-context/state blocks locally instead of consuming one shared operator-state model.
...
- **CLI shell:** Repo-root **`main.py`** is the compatibility entrypoint. It prepends `./src` when present, then calls **`obsidiandroid.common.repo_paths.ensure_repo_src_on_sys_path`**, then re-exports the canonical CLI implementation from **`obsidiandroid.cli.main`**.
- **Legacy trees** at the repo root are now primarily the reduced **`analysis/pipeline/`** compatibility tree plus repo-root **`database/`** shims. **`database/`** holds **thin shims** (and the **`split_db_health`** `__main__` entry) for legacy **`database.*`** paths; DB implementations for those names live under **`src/obsidiandroid/database/`** behind the curated **`obsidiandroid.database`** façade. The remaining migration burden is mostly **retirement / deprecation planning**, not discovery of missing canonical homes.
...
- **Quality gates:** **`make ci`** runs doc hygiene, **`scripts/dev/check_import_surface.py`** (imports, UTF-8 BOM scan, legacy leaf shim scan, database façade / legacy-shim identity via **`facade_manifest`**), fast pytest, and strict ML static scan.
- **Import-surface guard:** **`scripts/dev/check_import_surface.py`** rejects new imports from **`analysis`** or **`ml_classification`** inside canonical **`src/`** code, normal **`scripts/`**, and non-parity tests. Explicit shim/parity tests and the check script remain allowlisted because they verify legacy shim identity. The same guard rejects **`# Filename:`** headers in canonical **`src/`** whose first path segment names **`analysis`**, **`ml_classification`**, or repo-root **`database/`** (canonical DB sources live under **`src/obsidiandroid/database/`**), and enforces non-**`__init__`** legacy leaves under **`analysis/`** and **`ml_classification/`** as thin **`sys.modules`** identity shims.
- **Test intent split:** canonical package-surface checks now live separately from legacy shim parity checks (**`tests/test_obsidiandroid_package_surface.py`** vs **`tests/test_legacy_shim_parity.py`**). This keeps migration progress visible: new canonical surface tests should avoid legacy-root assumptions, while parity coverage remains explicit and easier to retire later.
...
- **Current working assumption:** Large new relocation passes are no longer the default. Prefer **small supportability passes** that either (a) remove overlap inside canonical **`src/`** code, (b) narrow legacy caller dependence, or (c) deprecate legacy output compatibility with explicit lifecycle labeling before removing it.
...
| P1 | **`README.md`** operator quickstart: **`pip install -e .`**, **`obsidiandroid`**, canonical vs legacy imports | **Improved** — Installation adds **Canonical package & imports** subsection with `obsidiandroid.pipeline` / `obsidiandroid.database` examples, CLI entrypoints, and pointer to `STRUCTURE_MIGRATION_PLAN.md`; deeper command cookbook still optional |
| P1 | **`analysis/pipeline`** (and adjacent **core** **`analysis/**`**) physical move under **`src/`** (keep legacy shims) | **Partial but functionally relocated** — Passes **66–71**, **74–76**, **78**, **80**, **81** moved the substantive implementation under **`obsidiandroid.*`**; matching **`analysis/*`** dirs are now mostly thin shims. The remaining work is **retirement planning**, **parity-test narrowing**, and documentation cleanup, not large physical moves. |
| P1 | **CLI/control-layer supportability** (reduce overlap inside canonical **`src/obsidiandroid/cli`**) | **Active** — Recent passes split diagnostics, parser state, and profile selection into smaller modules, but **`startup_menu.py`** remains large and several intent areas still rebuild local state/path logic. Next high-ROI slice: continue decomposing by operator intent and converge on one shared operator-state model. |
| P1 | **Erebus family/type authority view adoption** (**`v_android_sample_family_type_authority`**) | **Active** — read-only view now exists and is the current safe path for label-authority work. Next high-ROI slice: switch ObsidianDroid readiness/taxonomy diagnostics to consume the view directly, keep training behavior unchanged, and defer the broader `label_authority_*` foundation pack until the view-backed workflow is stable. |
...
| P2 | **`ml_classification/`** → **`obsidiandroid.modeling` / `features` / `labeling`** façades + **wrappers** where needed | **Done** — The repo-root **`ml_classification/`** compatibility tree is retired. New code should use **`obsidiandroid.*`** only. |
| P2 | **malware-family constants** | **Done** (**Pass 85**) — implementation now **`obsidiandroid.labeling.malware_family_constants`**; public callers should prefer wrapper functions from **`obsidiandroid.labeling.taxonomy`** unless raw taxonomy tables are explicitly needed |
| P2 | Vendor / evaluation domain boundary (**`obsidiandroid.vendors`**, **`obsidiandroid.evaluation`**) | **Canonical relocation complete** — implementations are canonical (Passes **59**, **63**, **64**) and stable entrypoints exist (**`obsidiandroid.vendors.parse_generic_classification`**, **`obsidiandroid.evaluation.parse_vendor_classifications`**) with CI guardrails. Remaining work is *contract freezing* (typed results / schema) before widening public API further. |
| P2 | **Legacy output compatibility retirement** (**`paper2_pack`**, duplicated split-audit exports, old **`paper_mode`** naming) | **Active / needs deprecation-first** — canonical run-scoped evidence and provenance now exist, but operator/support complexity remains high because legacy artifact names and mirrors still coexist with newer **`evidence_mode`** / **`publication_ready`** terminology. Next step: classify, label, and deprecate before removal. |
| P2 | **Logging engine debt closure** (stable `event_id`, explicit error severity, legacy short-name log retirement) | **Active** — canonical log names and `error.log` aggregate sink exist, but several call sites still lack `event_id` / explicit `level`, and legacy `analysis.log` / `database.log` / `export.log` / `menu.log` / `ml.log` / `pipeline.log` files should be pruned after one confirming run with the new names. |
| P2 | **Dead compatibility wrapper pruning** (private helpers and one-hop pass-throughs) | **Active** — several low-risk orphan helpers have already been removed from startup-menu and ablation modules. Next slice: continue trimming unused compatibility aliases and stale helper wrappers where there are no in-repo callers and no external API commitment. |
...
| P3 | Optional DB helpers (**`db_sample_timelines_queries`**, **`db_extract_av_label_keywords`**, …) — **canonical under `src/`**; some low-surface repo-root shims are now retired while the canonical modules remain on **`facade_manifest.FACADE_EXPORT_NAMES`** / package **`__all__`** for **`from obsidiandroid.database import …`** ergonomics | **Done** |
...
- **Pass 74 (permission trends helpers physical slice):** **`permission_trends/`** leaf modules (**`bundle_manifest`**, **`constants`**, **`publish_paths`**, **`reporting_support`**, **`sample_permission_data`**, **`stats_core`**) and **`permission_trends_selection`** are canonical under **`src/obsidiandroid/pipeline/`**; legacy **`analysis.pipeline.permission_trends.*`** imports still resolve to the same **`ModuleType`** objects; the final shell now registers those nested aliases from **`analysis/pipeline/__init__.py`** instead of keeping physical nested shim files on disk. **`obsidiandroid.pipeline.permission_trends`** façade imports canonical modules; **`check_import_surface`** asserts legacy **`analysis.pipeline.permission_trends.*`** matches canonical **`ModuleType`** objects.
- **Pass 75 (pipeline governance physical slice):** **`exceptions`**, **`integrity`**, **`policy`**, and **`readiness`** are canonical under **`src/obsidiandroid/governance/`**. **`obsidiandroid.governance`** package **`__init__.py`** registers all submodules from **`src`** (no **`analysis.pipeline.governance`** hop for façade contents). Legacy **`analysis.pipeline.governance.*`** imports still resolve to the same **`ModuleType`** objects; the final shell now registers those nested aliases from **`analysis/pipeline/__init__.py`** instead of keeping physical nested shim files on disk.
- **Pass 76 (pipeline manifest + artifacts physical slice):** **`hashing`**, **`writer`**, **`runtime_support`**, **`paper_compliance_checks`**, **`paper_figure_renderers`**, **`builder`**, and **`schema`** live under **`src/obsidiandroid/pipeline/manifest/`**; **`paths`** and **`registry`** under **`src/obsidiandroid/pipeline/artifacts/`**. legacy **`analysis.pipeline.manifest.*`** and **`analysis.pipeline.artifacts.*`** imports still resolve to the same **`ModuleType`** objects; the final shell now registers those nested aliases from **`analysis/pipeline/__init__.py`** instead of keeping physical nested shim files on disk. **`obsidiandroid.pipeline.manifest`** / **`.artifacts`** package **`__init__`** re-exports submodules; **`check_import_surface`** asserts legacy aliases match canonical modules.
...
- **Pass 80 (orchestration + matrix physical slice):** **`metadata_features`**, **`methodology_artifacts`**, **`permission_features`**, **`profile_filters`**, and **`runtime_reporting`** live under **`src/obsidiandroid/orchestration/`**; **`av_binary_matrix_builder`**, **`enrich_malicious_scores`**, and **`enrich_score_features`** under **`src/obsidiandroid/matrix/`**. The package-only **`analysis.orchestration`** and **`analysis.matrix`** shim namespaces are retired; runner, CLI, pipeline stages, and diagnostics imports use **`obsidiandroid.orchestration`** / **`obsidiandroid.matrix`**.
- **Pass 81 (risk band physical slice):** **`assign_risk_band`** and **`phase_score_engines`** live under **`src/obsidiandroid/risk_band/`**. The package-only **`analysis.risk_band`** shim namespace is retired; callers (`score_av_engines`, matrix enrichment) use **`obsidiandroid.risk_band`**.
...
- **Pass 83 (vectorization physical slice):** **`feature_encoder`**, **`feature_engine_selection`**, **`feature_vendor_extractor`**, and **`feature_vector_builder`** live under **`src/obsidiandroid/features/vectorization/`**. The package-only **`ml_classification.vectorization`** shim namespace is retired. Façade **`obsidiandroid.features`** re-exports the canonical **`obsidiandroid.features.vectorization.*`** modules.
...
- **Pass 85 (malware-family constants physical slice):** **`malware_family_constants`** lives under **`obsidiandroid.labeling`**. Canonical taxonomy/vendor parser callers use **`obsidiandroid.labeling.*`** paths.
...
- **Pass 87 (modeling utility physical slice):** **`distribution_reporter`** and **`feature_label_alignment_helper`** live under **`obsidiandroid.modeling`**; legacy **`ml_classification.ml_utils.*`** paths are thin identity shims. This was a small preparatory slice; later passes moved the training stack.
...
- **Pass 102 (utils bootstrap dedupe):** **`utils/__init__.py`** performs checkout **`sys.path`** bootstrap (**`src/`** prepend + **`ensure_repo_src_on_sys_path`**); the former **`utils/repo_import_paths.py`** shim was removed and folded here. **`scripts/dev/import_surface_policy.py`** enforces **`utils/__init__.py`** bootstrap invariants.
- **Pass 103 (main.py bootstrap shortcut):** Repo-root **`main.py`** uses **`import utils`** for **`utils/__init__.py`** bootstrap instead of referencing a dedicated legacy shim module.
- **Pass 104 (canonical import ratchet):** **`scripts/dev/check_import_surface.py`** now AST-scans canonical **`src/`** code and normal **`scripts/`** for direct imports from legacy compatibility roots (**`analysis`**, **`ml_classification`**, **`model`**, **`utils`**). The check script itself remains exempt because it intentionally imports legacy paths for shim identity verification.
- **Pass 105 (test/header ratchet):** **`scripts/dev/check_import_surface.py`** now AST-scans non-parity tests for direct legacy-root imports, with an explicit allowlist for shim/parity tests and fixtures, and rejects canonical **`src/`** modules whose **`# Filename:`** header still points at a legacy root. Current canonical source headers were refreshed to canonical paths where stale.
- **Pass 106 (legacy leaf shim ratchet):** **`scripts/dev/check_import_surface.py`** now enforces every non-**`__init__`** Python file under **`analysis/`**, **`ml_classification/`**, and **`model/`** as a thin canonical **`obsidiandroid.*`** module-alias shim with **`sys.modules`** identity registration. Package **`__init__`** files remain separately handled because they carry lazy accessors / package-level alias registration.
- **Pass 107 (import-surface checker maintainability):** **`scripts/dev/check_import_surface.py`** split import smoke, common/reporting/utils shim checks, observability/diagnostics/database shim checks, and static policy scans into helper functions. Behavior and checked surfaces are unchanged; the pass lowers the largest checker function size before adding more guardrails.
...
2. **Canonical surface** — **`obsidiandroid.*`** is the long-term **public/canonical** import surface. Root **`analysis.*`**, **`database.*`**, and **`ml_classification.*`** remain **implementation** paths until each domain has an appropriate façade and migration coverage.

3. **Breaking changes / shims** — No sudden flag day. **New and internal code** should prefer **`obsidiandroid.*`**. Legacy imports remain via **shims** during migration. Retire old paths only after: internal code no longer needs them; tests rely on them only where shim parity is the point; docs do not recommend them; and at least one stable checkpoint exists after the canonical path is real. For now: root **`main.py` stays** for operators, and remaining legacy trees stay until façades land.

4. **Tests** — **New tests should use canonical imports by default** (`obsidiandroid.*`, **`scripts.dev`**, **`scripts.diagnostics`** as already documented). Use **`main`**, or raw **`analysis.*` / `database.*`** only when testing shims, unavoidable monkeypatch surfaces, or migration parity explicitly.
...
| `utils/__init__.py` bootstrap | **moved_now** | Prepends repo `src/` when present then calls `obsidiandroid.common.repo_paths.ensure_repo_src_on_sys_path` (formerly via **`utils/repo_import_paths.py`**; that module removed). |
| `main.py` (root) | **wrapper_kept** | Uses **`import utils`** so checkout installs get **`src/`** on **`sys.path`** before **`obsidiandroid`** imports. |
| `src/obsidiandroid/common/__init__.py` | **moved_now** | Re-exports `ensure_repo_src_on_sys_path` (starts real `common` content). |
| `src/obsidiandroid/pipeline/__init__.py` | **moved_now** | Canonical pipeline namespace and public import surface; legacy **`analysis.pipeline.*`** imports are now compatibility aliases brokered by the final protected `analysis/pipeline` shell. |
...
| `obsidiandroid/pipeline/` | **moved_now** (canonical) | Canonical pipeline implementation and namespace live under **`src/obsidiandroid/pipeline/`**; **`analysis/pipeline/`** remains on disk primarily as a thin compatibility shim tree. |
...
| `obsidiandroid/database/` | **moved_now** (canonical) | Implementations live under **`src/obsidiandroid/database/`** and the curated façade is canonical. Repo-root **`database.*`** remains as thin compatibility shims; ordered exports + CI parity live in **`facade_manifest`**. |
| `obsidiandroid/vendors/` | **moved_now** (canonical) | **`parsing/`**, **`contracts/`** (incl. **`record_diagnostics`**, **`metadata_normalizer`** — Pass **84**), and **`execution/`** live under **`src/`**. Remaining work is API/boundary cleanup and eventual shim retirement, not physical relocation. |
| `obsidiandroid/features/` | **moved_now** | **`vectorization/`** canonical (**Pass 83**) plus **`feature_schema_audit`** at package root (**Pass 89**); legacy **`ml_classification.*`** feature/vectorization paths are shims. |
| `obsidiandroid/labeling/` | **moved_now** (canonical) | **`malware_family_constants`**, **`classification_label_resolver`**, (**Pass 95**) **`label_input_validator`**, **`label_builder_wrapper`**, **`label_postprocessor`**, **`label_field_normalizer`**, **`label_format_generator`**, and the **`taxonomy`** wrapper live under **`src/`**. Remaining **`ml_classification/labeling/`** paths are shim-only compatibility surfaces. |
...
| `obsidiandroid/modeling/` | **moved_now** | **`model_exporter`**, modeling helpers, **dataset_splitter**, and the **full default training execution surface** (**Passes 87–88, 90–94**) under **`src/`** including **`ml_trainers/`**; legacy **`ml_classification/training/`** and moved **`ml_classification.ml_utils`** names are shim-only. |
| `obsidiandroid/evaluation/` | **moved_now** (canonical) | AV/vendor tooling and classifier training eval/reporting (**`ml_eval_engine`**, **`ml_comparator_summary`**, **`accuracy_band_utils`**, **`ml_report_builder`**) are canonical under **`src/`**. Remaining work is boundary/API cleanup per **`docs/VENDOR_EVALUATION_BOUNDARY_PLAN.md`**, not physical migration. |
...
| `obsidiandroid/reporting/` | **moved_now** (canonical) | **`export_manager`**, LaTeX/family/confusion helpers, and **`compile_classification_results`** (**Pass 98**) are canonical under **`src/`**. The only remaining work is domain-boundary cleanup with **`obsidiandroid.evaluation`**, not code relocation. |
| `obsidiandroid/observability/` | **moved_now** (canonical) | **`logging/`** and **`pipeline_observability/`** are canonical under **`src/`**; package **`__init__`** provides the stable public re-exports. |
| `obsidiandroid/governance/` | **moved_now** (canonical) | Pipeline governance leaves (**exceptions**, **integrity**, **policy**, **readiness**), compliance, manifest, cohort, evidence mode, and artifacts live under **`src/`**. Remaining **`analysis.pipeline.governance.*`** paths are compatibility aliases only. |
| `obsidiandroid/common/` | **moved_now** (canonical) | Hashing, canonical CSV/SHA helpers, path safety, runtime diagnostics paths, output-path helpers, JSON IO, and checkout **`repo_paths`** live under **`src/obsidiandroid/common/`**. Remaining work is organization/doc cleanup, not relocation. |
...
| `utils/repo_import_paths.py` | *(removed)* | **retired** | Bootstrap folded into **`utils/__init__.py`**; **`import utils`** executes it. |
...
| `analysis/pipeline/permission_trends/*`, `permission_trends_selection.py` | `obsidiandroid.pipeline.*` | **moved_now** (**Pass 74**) — compatibility aliases now brokered by **`analysis/pipeline/__init__.py`**; no nested shim files required. |
| `analysis/pipeline/governance/*` | `obsidiandroid.governance` | **moved_now** (**Pass 75**) — **`exceptions`**, **`integrity`**, **`policy`**, **`readiness`** under **`src/`**; compatibility aliases now brokered by **`analysis/pipeline/__init__.py`**. |
...
| `model/vendor`, `model/parsing/`, `ml_classification/engine_weights/` | `obsidiandroid.vendors` / `obsidiandroid.engine_weights` | **moved_now** — vendor record contracts moved in Pass 60/84; engine weights moved in **Pass 98**; legacy paths are compatibility shims. Boundary rules remain in **`docs/VENDOR_EVALUATION_BOUNDARY_PLAN.md`**. |
...
| `utils/repo_import_paths.py` | **removed** — logic folded into **`utils/__init__.py`** (bootstrap remains outside **`src/`** so **`src/`** can be prepended before importing **`obsidiandroid`**). |
...
| Shim | Status |
|------|--------|
| `utils/hash_utils.py` | **wrapper_kept** — star-import from `obsidiandroid.common.hash_utils` (bootstrap via **`import utils`** / package **`__init__.py`**) |
...
| `utils/evidence_mode_resolver.py` | **wrapper_kept** — star-import from governance |
...
## Pass 16 (complete): `obsidiandroid.diagnostics` facade (historical, superseded)

| Item | Notes |
|------|-------|
| **`src/obsidiandroid/diagnostics/__init__.py`** | Historical early façade step. Current state is stronger: diagnostics implementation is canonical under **`src/obsidiandroid/diagnostics/`**. |
| **`scripts/dev/check_import_surface.py`** | Verifies the canonical diagnostics package surface. |
| **`tests/test_obsidiandroid_package_surface.py`** | **`test_diagnostics_facade_modules_match_analysis_diagnostics`**. |

This section is historical only. See **Snapshot — current truth** for the active state.
...
|----------------|-----------------|
| **`shim_only`** | **`hash_utils`**, **`canonicalization`**, **`display_distribution`**, **`ml_console`**, **`path_safety`**, **`runtime_paths`**, **`evidence_mode_resolver`**, **`prompt_utils`**, **`confusion_matrix_exporter`**, **`output_cleanup_clutter`**, **`av_detection_tiers`**, **`compliance`**, **`latex_tables`**, **`sample_metadata_preprocessor`**, **`pipeline_entry`**, **`exporting/naming`**, **`exporting/vendor_raw`**, **`exporting/workbook`**, **`menu/*`**, **`ui/*`**. |
...
| **`utils/model_exporter.py`** | **wrapper_kept** | Thin shim: **`import *`** from **`obsidiandroid.modeling.model_exporter`**. |
...
| **`utils/logging/`** | **wrapper_kept** — thin **`import *`** / re-export shims to canonical modules (no **`sys.modules`** alias). |
...
| **`scripts/dev/check_import_surface.py`** | **Thin shim policies:** repo-root **`utils/*.py`** (exclude **`export_manager`**, **`startup_menu`**, **`__init__.py`**), **`utils/exporting`** leaf modules (exclude aggregating **`__init__.py`**), **`utils/logging`** — max line counts, canonical substring, **`utils/__init__.py`** bootstrap check, no module-level **`def`/`class`**. **UTF-8 BOM scan:** no **`*.py`** under the repo (excluding skipped dirs) may start with a BOM — breaks **`ast.parse`** and tooling. |
...
| Item | Notes |
|------|-------|
| **`obsidiandroid.diagnostics`** | Early façade expansion step. Current state: these modules are canonical under **`src/obsidiandroid/diagnostics/`** and mirrored by **`analysis.diagnostics.*`** compatibility shims. |
| **`scripts/diagnostics/report_output_inventory.py`** | Canonical home; repo-root **`scripts/report_output_inventory.py`** is a thin compatibility wrapper. |
| **`scripts/diagnostics/report_feature_lineage.py`** | Canonical home; repo-root **`scripts/report_feature_lineage.py`** is a thin compatibility wrapper. |
| **`scripts/diagnostics/diagnose_alignment_gap.py`**, **`scripts/diagnostics/trace_feature_builder_drops.py`**, **`scripts/diagnostics/report_feature_matrix_gap.py`**, **`scripts/diagnostics/check_run_integrity.py`** | Canonical homes; repo-root names are thin compatibility wrappers. |
...
| Item | Notes |
|------|-------|
| **`obsidiandroid.diagnostics`** | Historical façade expansion. Current state: these modules are canonical under **`src/obsidiandroid/diagnostics/`** with **`analysis.diagnostics.*`** shim parity. |
...
| `obsidiandroid.modeling` | `distribution_reporter` | retired legacy package shim |
| `obsidiandroid.modeling` | `feature_label_alignment_helper` | retired legacy package shim |
| `obsidiandroid.features` | `feature_vector_builder` | retired legacy package shim |
...
("retired ml_classification.vectorization", "feature_vector_builder")
("ml_classification.labeling", "classification_label_resolver")
("retired ml_classification.ml_utils", "distribution_reporter")
("retired ml_classification.ml_utils", "feature_label_alignment_helper")
```
...
| Engine scoring / engine weights | **`obsidiandroid.evaluation`** / **`obsidiandroid.engine_weights`** | Engine weights are physically canonical under **`obsidiandroid.engine_weights`**; broader scoring policy remains research-sensitive. |
...
| Risk band config | **`obsidiandroid.risk_band`** | `moved_now`; legacy **`model.core`** / **`analysis.risk_band`** paths are shims. |
...
|---|---|---|
| **`normalize_family_name`**, **`is_known_family_name`**, **`canonicalize_family_label`** | Delegates to **`obsidiandroid.labeling.malware_family_constants`** | Wrapper functions are **not** the same objects as the underlying implementation callables (allows future indirection). |
...
- Vendors: stable entrypoint exists (**`obsidiandroid.vendors.parse_generic_classification`**). Remaining boundary work is *what else* becomes a public `obsidiandroid.vendors` API beyond contracts.
- Evaluation: stable entrypoint exists (**`obsidiandroid.evaluation.parse_vendor_classifications`**). Remaining work is freezing the return schema (or adding a typed result wrapper) before widening the public evaluation surface.
...
| 5 | **Legacy ML shim retirement** | ML implementation has moved, but legacy **`ml_classification`** package facades remain compatibility API. | **`ml_classification/*`** shims, package **`__getattr__`** facades, parity tests | New code could keep depending on legacy paths; sunset criteria unclear | Import-surface enforcement plus docs/caller cleanup; deliberate deprecation policy before removal | **ML** |
| 6 | **Vendor: parsing / records / scoring spread** | Vendor logic is now mostly canonical, but records, parser APIs, scoring summaries, and engine weights still need public contract decisions. | **`obsidiandroid.vendors/*`**, **`obsidiandroid.evaluation/vendor_*`**, **`obsidiandroid.engine_weights/*`**, legacy shims | Duplicated concepts and unclear public API boundaries | Boundary doc refresh + wrapper/API contract decisions | **Vendor** |
...
**Implementation home (Pass 38+):** façade-tier logic lives under **`src/obsidiandroid/database/`**; repo-root **`database/<name>.py`** are **identity shims** for legacy **`import database.<name>`** (except **`split_db_health`**, which also supports **`python -m database.split_db_health`**). **`obsidiandroid.database`** re-exports **the same modules** documented in tiers below; ordered names and CI parity tuples: **`obsidiandroid.database.facade_manifest`**.
...
| **Connections** | **`obsidiandroid.database.db_engine`** remains authoritative for pooled/direct connections and Permission Intel connectors. The façade does not alter construction. |
| **Configuration** | **`obsidiandroid.database.db_config`** holds **`OBSIDIAN_*`-backed constants** (optional dotenv load). **`obsidiandroid.database.settings`** exposes **`ObsidianConnectionSettings`** / **`load_connection_settings()`**. |
...
| **Future interop** (Erebus/Scytale, other backends) | **Out of scope** for Pass 38. Document **additive path**: evolve **`src/obsidiandroid/database/*`**, keep repo-root shims in lockstep, then widen **`obsidiandroid.database`** exports; **`ObsidianConnectionSettings`** may grow fields in a later pass — no premature service layer here. |
| **Logging / errors** | Re-export canonical **`obsidiandroid.database.db_errors`** alongside engine; **`db_engine`** observability unchanged. |

### Non-goals

- Physical relocation of façade **`database/`** modules under **`src/`** is **done**; the repo-root **`database/`** tree remains for import compatibility and **`python -m database.split_db_health`**.
...
Other **`database.db_*`** helpers (e.g. **`db_sample_timelines_queries`**, **`db_extract_av_label_keywords`**, **`db_av_engine_stats`**, **`db_av_disagreement_analysis`**) are **implemented under `src/obsidiandroid/database/`** and remain included in **`facade_manifest.FACADE_EXPORT_NAMES`** (package **`__all__`**). Some low-surface repo-root helper shims in that set have already been retired; canonical imports should use **`obsidiandroid.database.*`**.

### Enforcement & gates

| Gate | Requirement |
|------|----------------|
| **Import surface** | **`scripts/dev/check_import_surface.py`** asserts each façade export **`is`** the still-live **`database.*`** counterpart; optional helpers only participate when their repo-root compatibility shim is intentionally still active via **`facade_manifest.LEGACY_SHIM_PAIRS`**. Low-surface AV/scoring and utility/error helper shims have already moved out of that live legacy set. |
...
**Rollback:** Revert **`src/obsidiandroid/database/__init__.py`**, **`facade_manifest.py`**, and façade tests; **`database.*`** callers unaffected.

---

## Next pass suggestions

1. **Database surface:** **`obsidiandroid.database`** includes **Passes 38 + 43** façade exports; optional helpers are **canonical under `src/`** with repo-root shims (**`facade_manifest`**). **`python -m database.split_db_health`** remains a valid **`database`** module path for CLI entry. Optional: widen package **`__all__`** for the four optional helper modules when operators want **`from obsidiandroid.database import …`** ergonomics.
