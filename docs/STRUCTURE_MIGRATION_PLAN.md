...
- **CLI shell (Pass 103):** Repo-root **`main.py`** bootstraps checkout installs with **`import utils`** only; **`utils.__init__.py`** prepends **`src/`** when present then calls **`obsidiandroid.common.repo_paths.ensure_repo_src_on_sys_path`** (the former **`utils.repo_import_paths`** module was removed after folding bootstrap into **`utils.__init__.py`**).
- **Legacy trees** at the repo root (**`analysis/`**, **`ml_classification/`**, **`model/`**, **`utils/`**) stay primarily for compatibility shims and legacy import identity; repo-root **`database/`** remains an intentional implementation tree behind the curated **`obsidiandroid.database`** façade. **`utils/`** is primarily **shims** plus package **`__init__.py`** that performs checkout **`sys.path`** bootstrap (**Pass 102**), **`export_manager`** (**`sys.modules`** alias), and entry wrappers (**`startup_menu`**, **`pipeline_entry`**). In-repo scripts and tests should prefer **`obsidiandroid.common.repo_paths.ensure_repo_src_on_sys_path`** when they cannot rely on **`import utils`** ordering.
...
- **Import-surface guard:** **`scripts/dev/check_import_surface.py`** now rejects new imports from **`analysis`**, **`ml_classification`**, **`model`**, or **`utils`** inside canonical **`src/`** code, normal **`scripts/`**, and non-parity tests. Explicit shim/parity tests and the check script remain allowlisted because they verify legacy shim identity. The same guard rejects stale legacy **`# Filename:`** headers in canonical **`src/`** modules and enforces non-**`__init__`** legacy leaves under **`analysis/`**, **`ml_classification/`**, and **`model/`** as thin **`sys.modules`** identity shims.
