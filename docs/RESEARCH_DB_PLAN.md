...
### What it enables (future, not v2.2 implementation)
...
4. **Artifact parity** — every persisted row should trace to a canonical artifact path or an explicit derivation rule documented in `release_manifests`.
...
| **Source** | `profiles/*.yaml`, `label_contract_*.json` |
| **v2.2** | **Required** (seed rows for four canonical profiles) |
...
| **Source** | `ml_sample_label_fact_{run_id}.csv`, `label_contract_{run_id}.json` |
| **v2.2** | **Required** |
...
| **v2.2** | **Required** (after export exists) |
...
| **v2.2** | **Recommended** (audit-heavy profiles) |
...
| **v2.2** | **Required** (at least run-level flags) |
...
| **Key columns** | `release_tag` (e.g. `v2.2.0`); `profile_id`; `run_id`; `git_commit`; `git_tag`; `code_version`; `importer_version`; `imported_at_utc`; `manifest_json` (artifact paths, hashes, notes); PK (`release_tag`, `profile_id`) |
| **Source** | Git tag + canonical slot `run_manifest.json` + `ml_run_manifest_{run_id}.json` |
| **v2.2** | **Required** for documenting v2.2.0 backfill |
...
### Optional future tables (not v2.2)
...
## 4. canonical artifact importer (dry-run skeleton, v2.2.0)

**Script:** `scripts/import_canonical_runs_to_db.py`
**Mode:** dry-run only in v2.2.0 — validates run folders and prints an import plan; **no database writes**. Future `--apply` is deferred past v2.2.0.
...
4. `label_contract_{run_id}.json` → contract metadata on `runs.manifest_json` or future `contracts`
...
### Dry-run report (implemented in v2.2.0)
...
### Non-goals for importer v0 (v2.2.0)
...
**Recommended optional columns** (import-friendly, not blocking v2.2.0):
...
### Implementation path (deferred to v2.2.0+)
...
| `scripts/import_canonical_runs_to_db.py` | Dry-run skeleton — single slot (`--run-root`) or batch (`--runs-root`) |
| `tests/test_import_canonical_runs_to_db.py` | Contract tests for fixture slots and blocking gates |
| `make dry-run-canonical-db-import` | Batch dry-run on `artifacts/baselines/canonical_slots` |
| `docs/ML_SAMPLE_PERMISSION_FEATURE_EXPORT.md` | Standalone sparse export spec (implemented v2.2.0) |
| `artifacts/baselines/canonical_db_import_dry_run_fixture_slots.json` | Frozen batch dry-run baseline for fixture slots |
...
### Explicitly deferred past v2.2.0

- Live importer `--apply` and runtime pipeline DB writes during `stage_manifest`
- `ml_sample_permission_feature` pipeline export implementation (delivered v2.2.0)
- Web UI / Quasar
...
### Suggested next steps after v2.2.0 review

1. Apply DDL to a dev MySQL instance
2. Dry-run all four v2.2.0 canonical slots with `--release-tag v2.2.0`
3. ~~Implement `ml_sample_permission_feature` sparse export (v2.2.0)~~ **delivered in v2.2.0**
4. Add importer `--apply` behind explicit flag + DB preflight
5. Backfill `release_manifests` for `v2.2.0`

---

## 8. v2.2.0 milestone (delivered)
...
| `scripts/import_canonical_runs_to_db.py` | Counts `sample_permission_facts` when export exists; missing export = warning only |
| `tests/test_ml_seed_exports.py` | Export columns, present-only rule |
| `tests/test_import_canonical_runs_to_db.py` | Dry-run row counts + legacy-run warning behavior |
| `OBSIDIANDROID_RESEARCH_DB_*` env tuple | Documented; defaults mirror Erebus host/port/user/password |

### Locked design defaults (v2.2.0)
...
| `prediction_facts` | Recommended, not required for v2.2.0 backfill |
| Missing export on older runs | Warning only, not blocking |
| PI enrichment at export | Offline defaults (`unknown` / `aligned_features`) until v2.2.2+ |
...
## Appendix: v2.2.0 canonical runs (reference)
...
| Importer | Dry-run skeleton only in v2.2.0 |
...
4. **`prediction_facts` requirement** — recommended vs required for v2.2.0 canonical backfill.
...
4. ~~**`prediction_facts` requirement**~~ — **resolved:** recommended, not required for v2.2.0 backfill.
