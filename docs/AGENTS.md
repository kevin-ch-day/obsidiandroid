...
- Other entry points: `run.sh` / **`python -m obsidiandroid.cli.startup_menu`** (`obsidiandroid` console script → `obsidiandroid.cli.startup_menu:main`), ML training under **`obsidiandroid.modeling`** with repo-root **`ml_classification/`** kept as compatibility shims, and **database:** every module on the **`obsidiandroid.database`** façade (connection, engine, sample metadata, AV Tier D helpers, utilities, split health) is **canonical under `src/obsidiandroid/database/`**; repo-root **`database/<name>.py`** are **thin shims** (except **`split_db_health`**, which also supports **`python -m database.split_db_health`**). The ordered façade name list and derived legacy-shim parity tuples live in **`obsidiandroid.database.facade_manifest`** (single source for package bootstrap and import checks). Additional **`database.*`** helpers (timelines, AV disagreement, engine stats, label-keyword extraction) are **implemented under `src/`** with the same identity-shim pattern but are **not** necessarily on the package **`__all__`**. **Canonical DB imports** use **`obsidiandroid.database`**. Avoid ad-hoc re-exports beyond the façade.
...
- Top-level **`analysis/`** and **`ml_classification/`** are mostly **transitional compatibility/shim** trees; repo-root **`database/`** holds **thin shims** for façade exports and for **optional** query helpers whose implementations also live under **`src/obsidiandroid/database/`**. Repo-root **`utils/`** and **`model/`** were **removed** — use **`obsidiandroid.*`** (contracts under **`obsidiandroid.vendors.contracts`**, risk-band config under **`obsidiandroid.risk_band`**). Do **not** relocate or delete remaining legacy roots without a **dedicated migration pass** and test updates.
...
- **Do not move** core domains such as **`analysis/pipeline`**, **`database/`**, or **`ml_classification/`** as a drive-by—only with a planned pass and tests. **Note:** `analysis/pipeline` remains on disk primarily for **import compatibility** (thin shims); substantive pipeline code belongs in **`obsidiandroid.pipeline`**.
