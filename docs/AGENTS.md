...
- Other entry points: `run.sh` / **`python -m obsidiandroid.cli.startup_menu`** (`obsidiandroid` console script → `obsidiandroid.cli.startup_menu:main`), ML training under **`obsidiandroid.modeling`** with repo-root **`ml_classification/`** kept as compatibility shims, and **database:** every module on the **`obsidiandroid.database`** façade (connection, engine, sample metadata, AV Tier D helpers, utilities, split health) is **canonical under `src/obsidiandroid/database/`**; repo-root **`database/<name>.py`** are **thin shims** (except **`split_db_health`**, which also supports **`python -m database.split_db_health`**). Additional **`database.*`** helpers (timelines, AV disagreement, engine stats, label-keyword extraction) are **implemented under `src/`** with the same identity-shim pattern but are **not** necessarily on the package **`__all__`**. **Canonical DB imports** use **`obsidiandroid.database`**. Avoid ad-hoc re-exports beyond the façade.
