...
- Top-level **`analysis/`**, **`ml_classification/`**, **`model/`**, and **`utils/`** are mostly **transitional compatibility/shim** trees; top-level **`database/`** remains an intentional implementation tree behind the curated **`obsidiandroid.database`** façade. Do **not** relocate or delete legacy roots without a **dedicated migration pass** and test updates.
...
- **Import-surface enforcement:** `scripts/dev/check_import_surface.py` fails if canonical `src/` code or normal `scripts/` import legacy compatibility roots (`analysis`, `ml_classification`, `model`, `utils`). It also fails non-parity tests that import those roots directly, except explicit shim/parity fixtures, rejects stale legacy `# Filename:` headers in canonical `src/` modules, and enforces non-`__init__` legacy leaves under `analysis/`, `ml_classification/`, and `model/` as thin `sys.modules` identity shims. The check script itself is exempt because it deliberately imports legacy paths to verify shim identity.
