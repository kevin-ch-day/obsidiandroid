...
- Other entry points: `run.sh` / **`python -m obsidiandroid.cli.startup_menu`** (`obsidiandroid` console script → `obsidiandroid.cli.startup_menu:main`), ML training under **`obsidiandroid.modeling`** with repo-root **`ml_classification/`** kept as compatibility shims, and database **implementation**: **core connection/config/SQL helpers** are canonical under **`src/obsidiandroid/database/`** (**`cohort_sql_fragments`**, **`db_config`**, **`db_errors`**, **`schema_map`**, **`settings`**, **`db_engine`**, **`db_utils`**, **`split_db_health`**) with matching **`database.*`** shims (``split_db_health`` keeps ``python -m database.split_db_health``). **Query modules** (**`db_sample_metadata_*`**, AV/scoring façade tiers, …) remain under repo-root **`database/`** until a widen pass. **Canonical DB imports** use **`obsidiandroid.database`** (same module objects as **`database.<name>`** for façade-listed modules). Other **`database.db_*`** helpers remain **implementation-only** until **`STRUCTURE_MIGRATION_PLAN.md`** records a widen pass — avoid ad-hoc re-exports.
