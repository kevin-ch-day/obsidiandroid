...
- Top-level **`analysis/`** now effectively means the **`analysis/pipeline/`** compatibility tree; repo-root **`database/`** holds **thin shims** for every façade-aligned **`database.*`** module (implementations under **`src/obsidiandroid/database/`**). Repo-root **`utils/`**, **`model/`**, and **`ml_classification/`** were **removed** — use **`obsidiandroid.*`** (contracts under **`obsidiandroid.vendors.contracts`**, risk-band config under **`obsidiandroid.risk_band`**). Do **not** relocate or delete remaining legacy roots without a **dedicated migration pass** and test updates.
