...
- Top-level **`analysis/`** and **`ml_classification/`** are mostly **transitional compatibility/shim** trees; repo-root **`database/`** holds **thin shims** for every façade-aligned **`database.*`** module (implementations under **`src/obsidiandroid/database/`**). Repo-root **`utils/`** and **`model/`** were **removed** — use **`obsidiandroid.*`** (contracts under **`obsidiandroid.vendors.contracts`**, risk-band config under **`obsidiandroid.risk_band`**). Do **not** relocate or delete remaining legacy roots without a **dedicated migration pass** and test updates.
