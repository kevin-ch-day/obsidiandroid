...
- Other entry points: `run.sh` / **`python -m obsidiandroid.cli.startup_menu`** (`obsidiandroid` console script → `obsidiandroid.cli.startup_menu:main`), ML training under **`obsidiandroid.modeling`** with repo-root **`ml_classification/`** kept as compatibility shims, and **database:** every module on the **`obsidiandroid.database`** façade (connection, engine, sample metadata, AV Tier D helpers, utilities, split health) is **canonical under `src/obsidiandroid/database/`**; repo-root **`database/<name>.py`** are **thin shims** (except **`split_db_health`**, which also supports **`python -m database.split_db_health`**). Other **`database.*`** files (timelines, AV disagreement, …) stay as implementation until migrated. **Canonical DB imports** use **`obsidiandroid.database`**. Avoid ad-hoc re-exports beyond the façade.
...
- Top-level **`analysis/`** and **`ml_classification/`** are mostly **transitional compatibility/shim** trees; repo-root **`database/`** holds **thin shims** for every **`obsidiandroid.database`** façade export plus **non-façade** query helpers (timelines, AV disagreement, …) until migrated. Repo-root **`utils/`** and **`model/`** were **removed** — use **`obsidiandroid.*`** (contracts under **`obsidiandroid.vendors.contracts`**, risk-band config under **`obsidiandroid.risk_band`**). Do **not** relocate or delete remaining legacy roots without a **dedicated migration pass** and test updates.
