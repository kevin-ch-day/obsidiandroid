...
**Pass 63 update (code, not a re-audit):** Evaluation **implementation** modules that previously lived as **`analysis/evaluation/<module>.py`** files are now under **`src/obsidiandroid/evaluation/`**.

**Pass 64 update (code, not a re-audit):** Vendor parser **runtime** modules that previously lived as **`analysis/execution/<module>.py`** files are now under **`src/obsidiandroid/vendors/execution/`**. **Pipeline** physical layout has since also moved under **`src/obsidiandroid/pipeline/`**, with only the reduced **`analysis/pipeline/`** compatibility surface left on disk.

**Pass 65 update (code, not a re-audit):** Run **diagnostics** (including **`research_validity/`**, **`hostile_audit/`**, and leaf diagnostics modules) that previously lived under **`analysis/diagnostics/`** are now under **`src/obsidiandroid/diagnostics/`**.
...
**Plan refresh (2026-05, documentation only):** Passes **59** / **63** / **64** / **65** mean **`analysis/vendor_processing/`**, **`analysis/evaluation/`**, **`analysis/execution/`**, and **`analysis/diagnostics/`** are no longer live repo-root trees. Implementations live under **`src/obsidiandroid/`**. The **Import inventory** table below remains the **Pass 50B static snapshot** (useful for *who imports whom*); **Current source path** for those modules should be read as “**historical import prefix; implementation is under `obsidiandroid.…`**.” A full row-by-row rescan is optional.
