#!/usr/bin/env python3
"""Dry-run V3 run folder validation and import-plan reporter (no DB writes)."""
...
def build_import_plan(
    ...
    *,
    ...
    if plan.profile_id and not is_canonical_v3_profile(plan.profile_id):
        plan.warnings.append(f"profile_id={plan.profile_id} is outside the four canonical V3 profiles")
...
def dry_run_all_slots(
    *,
    ...
    """Build dry-run import plans for all canonical V3 profile slots under runs_root."""
...
def dry_run_all_slots_cli(
    *,
    ...
    """Dry-run import plans for all canonical V3 profile slots under runs_root."""
    exit_code = 0
...
def _format_plan(plan: ImportPlan, *, release_tag: str) -> str:
    lines: list[str] = []
    lines.append("ObsidianDroid V3 import plan (dry-run)")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate a V3 run folder and print a dry-run database import plan."
    )
    parser.add_argument(
        "--run-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Parent of canonical slot dirs; dry-runs all four V3 profiles.",
    )
    parser.add_argument(
        "--skip-missing-slots",
        action="store_true",
    ...
    parser.add_argument(
        "--release-tag",
        default="",
        help="Optional release tag (e.g. v3.0.0) for release_manifests row planning.",
    )
    parser.add_argument(
        "--allow-mixed",
        action="store_true",
