#!/usr/bin/env python3
"""Wait for a canonical run slot to finalize, then validate (and optionally refresh handoff)."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--refresh-handoff",
        action="store_true",
        help="Run refresh-canonical-handoff for the slot after manifest appears.",
    )
    parser.add_argument(
        "--validate-all",
        action="store_true",
    ...
    if args.refresh_handoff:
        from scripts.dev import refresh_canonical_handoff  # noqa: E402

        refresh_canonical_handoff.refresh_slot(
            runs_root=runs_root,
            profile_id=args.profile_id,
        )

    from scripts.dev import validate_canonical_runs as canonical_validate  # noqa: E402

    if args.validate_all:
        code = canonical_validate.verify_only_cli(
            runs_root=runs_root,
            strict=True,
            skip_missing_slots=True,
        )
    else:
        result = canonical_validate._verify_slot_profile(
            profile_id=args.profile_id,
            runs_root=runs_root,
            strict=True,
        )
