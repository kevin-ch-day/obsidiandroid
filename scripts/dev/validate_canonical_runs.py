#!/usr/bin/env python3
"""Validate canonical closure artifacts for the four canonical profiles."""
...
CANONICAL_PROFILES = tuple(CANONICAL_PROFILES)
...
def _verify_run(
    ...
    *,
    ...
    label_contract = diag / f"label_contract_{run_id}.json"
    ...
    dl_handoff = observability.get("dl_handoff")
    if isinstance(dl_handoff, dict):
        for key in (
            "ml_run_manifest",
            "ml_sample_label_fact",
            "ml_permission_vocabulary",
            ...
            ref = str(dl_handoff.get(key, "") or "").strip()
            if ref and _resolve_reference_path(ref, repo_root=REPO_ROOT) is None:
                caveats.append(f"dl_handoff path missing on disk: {key}")
        if strict and dl_handoff.get("dl_seed_status") != "ready":
            caveats.append(
                "run_observability_summary dl_handoff dl_seed_status is not ready "
                f"(status={dl_handoff.get('dl_seed_status')!r}; rerun finalize or make refresh-canonical-handoff)"
            )
    if strict and not isinstance(dl_handoff, dict):
        caveats.append("run_observability_summary missing dl_handoff block (rerun manifest finalize)")
    handoff_summary_path = diag / f"dl_handoff_summary_{run_id}.json"
    ...
    if handoff_summary_path.is_file():
        if handoff_summary.get("dl_seed_status") != "ready":
            caveats.append(
                "dl_handoff_summary reports incomplete DL seed handoff: "
                + "; ".join(str(item) for item in (handoff_summary.get("caveats") or []))
            )
    elif strict:
        caveats.append("missing dl_handoff_summary export (rerun manifest finalize or make refresh-canonical-handoff)")
...
def run_profiles_cli() -> int:
    ...
    for profile_id in CANONICAL_PROFILES:
        print(f"[canonical] Running profile={profile_id} …", flush=True)
        code = run_pipeline(
        ...
        if int(code) != 0:
            print(f"[canonical] profile={profile_id} exit_code={code}", file=sys.stderr)
