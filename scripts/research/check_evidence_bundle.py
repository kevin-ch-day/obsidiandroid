...
REQUIRED_DIAGNOSTICS_FILES = (
    ...
    "split_freeze_audit_{run_id}.csv",
    ...
    *,
    ...
    if not bundle_only:
        ...
        for row in paper_registry_rows:
            ...
            # Destination path is paper-facing and must not include unstable naming.
            if any(token in destination_path for token in disallowed_name_tokens):
                bad_registry_path_tokens.append(destination_path)
            # Source path may legitimately use bundle *.latest names; only block explicit fallback stems.
