"""Compact label contract artifact for governed classification runs.
...
def build_label_contract(
    *,
    ...
    """Build the compact label contract payload for one prepared cohort."""
...
def export_label_contract(
    *,
    ...
    """Write run-scoped label contract artifacts and return their paths."""
    ...
    md_path = diagnostics_dir / f"label_contract_{run_id}.md"
    json_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    oh.mirror_json_text_run_then_global(
        diagnostics_dir=diagnostics_dir,
        run_filename=json_path.name,
        payload=payload,
        global_latest_name="label_contract.latest.json",
    )
    ...
    oh.mirror_utf8_text_run_then_global(
        ...
        text=md_text,
        global_latest_name="label_contract.latest.md",
    )
