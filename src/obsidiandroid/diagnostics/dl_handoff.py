"""Deep-learning handoff summary for operators and offline validation."""
...
def build_dl_handoff_summary_payload(
    *,
    ...
    return {
        "summary_version": "dl_handoff_summary_v1",
        "run_id": run_id,
...
def build_dl_handoff_observability_block(
    *,
    diagnostics_dir: Path,
    run_id: str,
    manifest: dict[str, Any],
    ...
    """Build the ``dl_handoff`` block for run observability summaries."""
    diagnostics_dir = Path(diagnostics_dir)
    ctx = manifest_context if isinstance(manifest_context, dict) else {}
    ...
    return {
        ...
        "dl_handoff_summary": portable_repo_path(handoff_path),
    }


def export_dl_handoff_summary(
    *,
    diagnostics_dir: Path,
    run_id: str,
    profile: dict[str, Any],
    ...
    """Write run-scoped DL handoff summary JSON."""
    diagnostics_dir = Path(diagnostics_dir)
    diagnostics_dir.mkdir(parents=True, exist_ok=True)
    payload = build_dl_handoff_summary_payload(
        diagnostics_dir=diagnostics_dir,
        run_id=run_id,
        profile=profile,
        manifest=manifest,
    ...
    path = diagnostics_dir / f"dl_handoff_summary_{run_id}.json"
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    oh.mirror_json_text_run_then_global(
        diagnostics_dir=diagnostics_dir,
        run_filename=path.name,
        payload=payload,
        global_latest_name="dl_handoff_summary.latest.json",
    )
    return path
