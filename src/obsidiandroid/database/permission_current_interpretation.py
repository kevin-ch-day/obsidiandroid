...
def interpret_permission_evidence(
    *, token: str, identity: object | None = None, legacy: object | None = None,
    ...
    historical = (
        ...
        or catalog_lifecycle in {"historical", "legacy_removed", "removed"}
    )
    ...
    elif fact_source in {"chromium_source", "androidx_manifest"} or fact_scope == "custom_permission_pattern":
...
def interpretation_joins(*, key_expr: str, raw_expr: str) -> str:
    ...
    return f"""
        ...
        LEFT JOIN android_permission_v1_current_permission pi
          ON BINARY pi.canonical_permission = BINARY TRIM({raw_expr})
...
def interpretation_selects(*, historical_source_expr: str) -> dict[str, str]:
    """Return compatible SQL projections whose certainty follows evidence."""

    platform = f"pi.authority_class IN ({AOSP_AUTHORITIES_SQL})"
    historical = (
        f"(paf.fact_scope = 'removed_api' OR paf.lifecycle_status IN ({HISTORICAL_LIFECYCLES_SQL}) "
    ...
    third_party = "(paf.authority_source_type IN ('sdk_vendor_docs','sdk_inventory','chromium_source','androidx_manifest') OR paf.fact_scope = 'custom_permission_pattern')"
    historical_fact = f"""(paf.fact_scope = 'removed_api' OR (
    ...
    scope = f"""CASE
        ...
        WHEN paf.authority_source_type IN ('sdk_vendor_docs','sdk_inventory') THEN 'SDK_INTEGRATION_CUSTOM_PERMISSION'
    ...
    kind = f"""CASE
        ...
        WHEN {package} OR {third_party} OR {historical_fact} THEN 'PERMISSION'
        ELSE 'UNKNOWN' END"""
    declaration = f"""CASE
        ...
        WHEN {provider} OR {package} OR {third_party} THEN 'UNCONDITIONAL'
    ...
    evidence = f"""CASE
        ...
        WHEN paf.authority_fact_id IS NOT NULL AND ({provider} OR {package} OR {third_party} OR {historical_fact}) THEN 'SOURCE_BACKED_DEFINITION'
