...
def test_build_label_contract_distinguishes_canonical_profiles() -> None:
    df = _sample_df()
    major = label_contract.build_label_contract(
        profile={
    ...
    all_current = label_contract.build_label_contract(
        profile={
...
def test_build_label_contract_uses_type_namespace_for_type_taxonomy_profile() -> None:
    payload = label_contract.build_label_contract(
        profile={
...
def test_label_contract_marks_retired_type_rows_non_targetable() -> None:
    df = _sample_df()
...
def test_export_label_contract_writes_json_and_markdown(tmp_path: Path) -> None:
    paths = label_contract.export_label_contract(
    ...
    assert len(paths) == 2
    md_text = (tmp_path / "label_contract_runx.md").read_text(encoding="utf-8")
