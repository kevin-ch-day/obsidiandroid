"""Tests for the V3 dry-run database import planner."""
...
BASELINE_JSON = Path("artifacts/baselines/v3_db_import_dry_run_fixture_slots.json")

from obsidiandroid.common.run_slots import _SLOT_BY_PROFILE
from scripts import import_v3_run_to_db as importer

pytestmark = pytest.mark.contract

FIXTURES_ROOT = Path("artifacts/baselines/v3_canonical_slots")
...
def _write_minimal_slot(
    ...
    *,
    ...
    for stem in ("v3_label_contract", "permission_pattern_contract"):
...
def test_fixture_slots_produce_unblocked_dry_run_plan(slot_name: str) -> None:
    ...
    plan = importer.build_import_plan(slot_root, release_tag="v3.0.0")
...
def test_dry_run_all_slots_cli_reports_ready_for_minimal_tree(tmp_path: Path) -> None:
    ...
    for idx, profile_id in enumerate(importer.CANONICAL_V3_PROFILES):
        _write_minimal_slot(
            runs_root,
            profile_id=profile_id,
    ...
    code = importer.dry_run_all_slots_cli(runs_root=runs_root, release_tag="v3.0.0")
...
def test_import_plan_to_dict_round_trip_fields(tmp_path: Path) -> None:
    ...
    plan = importer.build_import_plan(slot_root, release_tag="v3.0.0")
    payload = importer.import_plan_to_dict(plan, release_tag="v3.0.0")
