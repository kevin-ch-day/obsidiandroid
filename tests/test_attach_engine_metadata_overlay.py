...
def test_attach_engine_metadata_slot_root_stays_run_scoped(monkeypatch, tmp_path) -> None:
    output_root = tmp_path / "output"
    ...
    run_scoped = diagnostics_dir / "engine_metadata_overlay_rid_slot.csv"
    global_latest = output_root / "diagnostics" / "engine_metadata_overlay.latest.csv"
    assert run_scoped.is_file()
    assert not (output_root / "diagnostics" / run_scoped.name).exists()
    assert not (diagnostics_dir / "engine_metadata_overlay.latest.csv").exists()
