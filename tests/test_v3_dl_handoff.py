"""Tests for V3 DL handoff summary exports."""
...
from obsidiandroid.diagnostics import v3_dl_handoff

pytestmark = pytest.mark.contract


def test_build_v3_dl_handoff_summary_reports_ready_when_seed_chain_complete(tmp_path: Path) -> None:
    run_id = "run_handoff"
    ...
    payload = v3_dl_handoff.build_v3_dl_handoff_summary_payload(
...
def test_build_v3_dl_handoff_summary_flags_missing_split_export(tmp_path: Path) -> None:
    run_id = "run_split"
    diagnostics_dir = tmp_path / "diagnostics"
    diagnostics_dir.mkdir(parents=True)
    (diagnostics_dir / f"ml_run_manifest_{run_id}.json").write_text(
...
def test_build_v3_dl_handoff_observability_block_mirrors_summary_fields(tmp_path: Path) -> None:
    ...
    block = v3_dl_handoff.build_v3_dl_handoff_observability_block(
        diagnostics_dir=diagnostics_dir,
        run_id=run_id,
    ...
    assert block["v3_dl_handoff_summary"].endswith(f"v3_dl_handoff_summary_{run_id}.json")
