...
def test_merge_sample_metadata_features_rejects_duplicate_sample_ids_in_evidence_mode(
    ...
    with pytest.raises(ValueError, match="Duplicate sample_id"):
        stage_feature_enrichment.merge_sample_metadata_features(
            enriched,
            samples,
...
def test_merge_sample_metadata_features_allows_duplicate_override_in_evidence_mode(
    monkeypatch, tmp_path
) -> None:
    monkeypatch.setattr(app_config, "RUNTIME_DIAGNOSTICS_DIR", str(tmp_path), raising=False)
    ...
    out = stage_feature_enrichment.merge_sample_metadata_features(
        enriched,
        samples,
        {"enable_sample_metadata_features": True},
        None,
