...
def test_stratified_kfold_for_grid_search_respects_cv_folds_and_support(monkeypatch) -> None:
    ...
    cv = parallel_layout.stratified_kfold_for_grid_search(10, random_state=42)
    assert cv is not None
    assert cv.n_splits == 5

    cv2 = parallel_layout.stratified_kfold_for_grid_search(2, random_state=0)
...
def test_stratified_kfold_coerces_cv_folds_below_two(monkeypatch) -> None:
    ...
    cv = parallel_layout.stratified_kfold_for_grid_search(6, random_state=0)
    assert cv is not None
    assert cv.n_splits == 2

    monkeypatch.setattr(app_config, "CV_FOLDS", 7, raising=False)
    cv = parallel_layout.stratified_kfold_for_grid_search(3, random_state=1)
    assert cv is not None
...
def test_stratified_kfold_treats_cv_folds_none_as_default(monkeypatch) -> None:
    ...
    cv = parallel_layout.stratified_kfold_for_grid_search(10, random_state=0)
