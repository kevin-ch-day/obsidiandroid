"""Integration tests for canonical V3 hard-fail export policies."""
...
def test_canonical_hygiene_bundle_reraises_ml_seed_failures(
    ...
    (diagnostics_dir / f"v3_label_contract_{run_id}.json").write_text("{}", encoding="utf-8")
...
def test_finalize_output_hygiene_bundle_refreshes_v3_label_contract(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    payload = json.loads((diagnostics_dir / f"v3_label_contract_{run_id}.json").read_text(encoding="utf-8"))
