"""Tests for offline V3 canonical run validation."""
...
from scripts.dev import validate_v3_canonical_runs as v3_validate
        ...
        json.dumps(
            {
                ...
                "v3_dl_handoff": {
                    ...
                    "v3_dl_handoff_summary": str(diagnostics_dir / f"v3_dl_handoff_summary_{run_id}.json"),
                },
            }
        ),
        encoding="utf-8",
...
def test_verify_only_passes_for_minimal_canonical_slot(tmp_path: Path) -> None:
    ...
    for idx, profile_id in enumerate(v3_validate.CANONICAL_PROFILES):
        _write_minimal_slot(
            runs_root,
    ...
    code = v3_validate.verify_only_cli(runs_root=runs_root)
...
def test_verify_only_reads_vocabulary_from_bundle_contracts_only(tmp_path: Path) -> None:
    ...
    summary = v3_validate._verify_slot_profile(profile_id=profile_id, runs_root=runs_root)
...
def test_verify_run_flags_stale_ml_run_manifest_vocabulary_count(tmp_path: Path) -> None:
    ...
    summary = v3_validate._verify_run(profile_id, run_id, diagnostics_dir=diagnostics_dir, run_root=slot_root)
    ...
    assert summary["ok"] is True
    strict_summary = v3_validate._verify_run(
        profile_id,
        run_id,
...
def test_verify_only_fails_when_required_contract_missing(tmp_path: Path) -> None:
    ...
    (slot_root / "diagnostics" / f"v3_label_contract_{all_current_run_id}.json").unlink(missing_ok=True)
...
def test_verify_only_strict_fails_when_observability_dl_seed_status_not_ready(tmp_path: Path) -> None:
    ...
    obs_payload["v3_dl_handoff"]["dl_seed_status"] = "incomplete"
    obs_path.write_text(json.dumps(obs_payload), encoding="utf-8")

    code = v3_validate.verify_only_cli(runs_root=runs_root, strict=True)
...
def test_verify_only_strict_fails_when_observability_dl_seed_status_missing(tmp_path: Path) -> None:
    ...
    obs_path = slot_root / "diagnostics" / "run_observability_summary.json"
    obs_payload = json.loads(obs_path.read_text(encoding="utf-8"))
    obs_payload["v3_dl_handoff"].pop("dl_seed_status", None)
    obs_path.write_text(json.dumps(obs_payload), encoding="utf-8")
