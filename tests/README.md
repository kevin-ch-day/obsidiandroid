...
- **Default run:** `pytest` / `make test` — respects **`pyproject.toml`** `[tool.pytest.ini_options]` (`-m "not (slow or integration or heavy or contract)"`). See `tests/conftest.py` for the `_SLOW_TEST_MODULES` list and temp output routing. Pytest uses its OS-managed temporary base, avoiding a shared repository-local test directory.
