...
FROM erebus_threat_intel_prod.android_malware_family WHERE family_id = 261;

SELECT COUNT(DISTINCT m.sample_id) AS catalog_mapped_samples,
       COUNT(DISTINCT a.sample_id) AS authority_samples,
       COUNT(*) AS authority_rows, SUM(a.family_id = 261) AS rows_mapped_to_family_261
