...
FROM erebus_threat_intel_prod.malware_sample_family_mapping WHERE family_id = 265;

SELECT
    (SELECT COUNT(*) FROM erebus_threat_intel_prod.android_malware_family_alias WHERE family_id = 265) AS alias_rows,
    (SELECT COUNT(*) FROM erebus_threat_intel_prod.android_malware_family_alias WHERE family_id = 265 AND is_active = 1) AS active_alias_rows,
    (SELECT COUNT(*) FROM erebus_threat_intel_prod.v_android_sample_family_type_authority WHERE family_id = 265) AS authority_rows,
    (SELECT COUNT(*) FROM (
      SELECT LOWER(TRIM(family_slug)) AS token FROM erebus_threat_intel_prod.android_malware_family WHERE is_active = 1 AND family_id <> 265
      UNION ALL SELECT LOWER(TRIM(alias_name)) FROM erebus_threat_intel_prod.android_malware_family_alias WHERE is_active = 1
    ) AS active_tokens WHERE token = 'fictus') AS active_slug_or_alias_collisions;
