...
FROM erebus_threat_intel_prod.android_malware_family WHERE family_id = 85;
...
LEFT JOIN erebus_threat_intel_prod.v_android_sample_family_type_authority AS a
  ON a.sample_id = m.sample_id AND a.family_id = 85
WHERE m.family_id = 85;

SELECT 'active_family_inactive_type_gaps_for_smsworm' AS check_name, COUNT(*) AS finding_count
FROM erebus_threat_intel_prod.android_malware_family AS f
JOIN erebus_threat_intel_prod.android_malware_type AS t ON t.type_id = f.primary_type_id
WHERE f.family_id = 85 AND f.is_active = 1 AND t.is_active = 0
UNION ALL
SELECT 'active_aliases_on_inactive_families', COUNT(*)
