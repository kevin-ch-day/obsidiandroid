...
SELECT
    COUNT(DISTINCT m.sample_id) AS catalog_label_samples,
    COUNT(DISTINCT a.sample_id) AS authority_samples,
    COUNT(*) AS authority_rows,
    SUM(a.family_id = 268) AS rows_mapped_to_family_268
FROM erebus_threat_intel_prod.malware_sample_catalog AS m
LEFT JOIN erebus_threat_intel_prod.v_android_sample_family_type_authority AS a
  ON a.sample_id = m.sample_id
WHERE LOWER(TRIM(m.family_label)) = 'jsmshider';
